...
<HARD-GATE>
Before proceeding:
1. `.specs/NNN-feature-name/tasks.md` must exist — if not, stop and invoke `$tasks`
...
6. **After all tasks are checked** — resolve the plugin package root from this skill's installed `SKILL.md` path in Codex's skill inventory, then run the bundled `<plugin-root>/scripts/check-complete.sh .specs/NNN-feature-name/tasks.md`. Do not look for a project-local copy of the helper.
...
Tell the user: "All tasks complete. Feature `NNN-feature-name` is done. Acceptance scenarios verified. Ready to run `$converge`."
